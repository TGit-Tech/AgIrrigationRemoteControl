...
//#define HAND_REMOTE               3     // Un-comment for the 3rd Hand-Remote
//#define HAND_REMOTE               4     // Un-comment for the 4th Hand-Remote
//#define HAND_REMOTE               5     // Un-comment for the 5th Hand-Remote
...
void setup() {
    ...
    Menu.Setup(THISDEVICE, XBEECONFIG, 12, 11 );  // Rx,Tx to 12, 11
  #else
    Menu.Setup(THISDEVICE, XBEECONFIG, 3, 2);     // Rx,Tx to 3, 2
  #endif
 ...
 * For PID   Controls(  _OutPin,        PID_SET,          _ID,        OnDevice, Kp, Ki, Kd, POn, PIDDirection, _StorePin = NULL)
...
#if THISDEVICE<6                  //vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
  Menu.AddPin(&PidControl);       // Menu items only on a Hand-Remote
  Menu.AddPin(&Battery);          //
#endif                            // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  Menu.Begin(&PumpPower);         // Which Pin the Menu Begins on
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
